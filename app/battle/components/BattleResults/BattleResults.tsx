...
const BattleResults = ({ playerOne, playerTwo }: BattleResultsProps) => {
    ...
    const coinReward = Math.floor(opponent.rewards.coin * ((playerOne.battleScore - playerTwo.battleScore) / 2 + 1))

    const xpReward = Math.floor(opponent.rewards.xp * ((playerOne.battleScore - playerTwo.battleScore) / 2 + 1))
    ...
    const handleResult = async (resultType: string) => {
        ...
        if (resultType === "win" && randomRewardChance < opponent.rewards.items[0].chance / 100) {
            const rewardItem = allItems.filter((item) => opponent.rewards.items[0].name.includes(item.name))
            setTimeout(async () => {
    ...
    return (
        <div className='battle-over fill center'>
            <img className='results-frame abs-center' src={resultsFrame.src} alt='results-frame' />
                                ...
                                <div className='rewards__bottom around'>
                                    ...
                                    {itemReward && (
                                        <div className='item-reward center'>
                                            <p>+</p>
                                            <img src={itemReward[0].image} alt='item reward' />
                                        </div>
                                    )}
                                </div>
                            </>
