...
const Item = ({ item, index, chosenItem, setChosenItem, allItems }: ItemComponent) => {
    const itemClasses = (item: IItem) => classSet("item", "between", chosenItem === item ? "chosen" : "")

    return (
        <div onClick={() => setChosenItem(allItems[index])} className={itemClasses(item)}>
            <div className='section center'>
                <img src={item.image} alt='item image' />
                <div className='item-name'>{item.name}</div>
            </div>
            <div className='item-price center'>
                {item.price} <img src={coinImage.src} alt='coin' />
            </div>
        </div>
    )
}
