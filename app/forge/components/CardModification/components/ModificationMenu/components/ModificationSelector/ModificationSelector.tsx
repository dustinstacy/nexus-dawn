...
const ModificationSelector = ({ selectedModification, setSelectedModification }: ModificationSelector) => {
    const user = useUserStore((state) => state.user)
    const allItems = useItemsStore((state) => state.allItems)

    const userModifiers = user?.inventory.filter((item) => item.type === "modifier").sort((a, b) => a.level - b.level)
